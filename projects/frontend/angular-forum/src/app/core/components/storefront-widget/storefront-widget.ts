    ...
    template: `
        <div class="storefront-widget rounded-2xl bg-[#1B2838] p-6 shadow-xl" *transloco="let t">
            ...
            @if (featuredItems().length > 0) {
                <div class="mb-6" (mouseenter)="pauseRotation()" (mouseleave)="resumeRotation()">
                    <h3 class="mb-3 text-sm font-bold tracking-wider text-white/70 uppercase">
                    ...
                    <div class="grid grid-cols-12 gap-3" style="height: 320px">
                        ...
                        <div class="relative col-span-8 overflow-hidden rounded-lg">
                            @if (activeItem(); as item) {
                                <img class="h-full w-full object-cover" [alt]="item.title" [src]="item.imageUrl" />
                                <div
                                ...
                                <div class="absolute right-0 bottom-0 left-0 p-5">
                                    ...
                                    @if (item.description) {
                                        <p class="mt-1 line-clamp-2 text-sm text-white/70">{{ item.description }}</p>
                                    }
                        ...
                        <div class="col-span-4 flex flex-col gap-1">
                            @for (item of featuredItems(); track item.id; let i = $index) {
                                <div
                                    ...
                                    [class]="
                                        activeIndex() === i
                                            ? 'border-l-2 border-blue-400 bg-[#1a2940]'
                                            : 'bg-[#0a0f16] hover:bg-[#1a2940]'
                                    "
                                ...
                                >
                                    ...
                                    <span class="line-clamp-1 text-sm font-medium text-white/90">{{ item.title }}</span>
                                </div>
                            }
                        </div>
                    </div>
                ...
                <div>
                    ...
                    <div class="storefront-scrollbar flex gap-3 overflow-x-auto pb-2">
                        @for (item of discountItems(); track item.id) {
                            <a class="group w-44 shrink-0" [routerLink]="item.linkUrl ? [item.linkUrl] : null">
                                ...
                                <div class="mt-2">
                                    <div class="line-clamp-1 text-sm font-medium text-white/90">{{ item.title }}</div>
